...
export default function Sidebar() {
  ...
  return (
    <div className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col h-full shrink-0">
      ...
      {/* Workspace Display */}
      {activeWorkspace && (
        <div className="mx-3 my-2.5 p-3 rounded-xl bg-slate-950/60 border border-slate-800/80 flex items-center justify-between">
          <div className="flex items-center gap-2 overflow-hidden">
            <FolderClosed className="w-4 h-4 text-violet-400 shrink-0" />
            <span className="text-xs font-semibold text-slate-200 truncate" title={activeWorkspace}>
              {getWorkspaceName()}
            </span>
          </div>
          <div className="flex items-center gap-1 text-[10px] text-slate-400 font-semibold px-2 py-0.5 rounded-full bg-slate-800">
      ...
      <div className="flex px-3 py-1.5 border-b border-slate-850 gap-1 bg-slate-950/20">
        <button
          ...
          className={`flex-1 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition ${
            activeExplorerTab === 'chat'
              ...
              : 'text-slate-500 hover:bg-slate-800/40 hover:text-slate-350 border border-transparent'
          }`}
        >
          Chat
        </button>
        ...
        <>
          ...
          <div className="px-3 py-1.5 relative font-sans">
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 bg-slate-950 border border-slate-850 rounded-xl text-xs text-slate-300 focus:outline-none focus:border-violet-500 transition shadow-inner"
            />
            <Search className="w-3.5 h-3.5 text-slate-500 absolute left-6 top-3.5" />
          </div>
      ...
      {/* Sidebar Footer */}
      <div className="p-3 border-t border-slate-850 flex flex-col gap-2 bg-slate-900/50">
        ...
        <button
          ...
          className="w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs font-semibold text-slate-300 hover:bg-slate-800/60 hover:text-white transition select-none"
        >
